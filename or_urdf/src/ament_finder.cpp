/** \file ament_finder.h
 * \brief Utility to resolve package to file URIs in ament workspaces
...
static const std::string AMENT_MARKER(".ament_marker");
  ...
  std::unordered_map<std::string, std::string>& _packageMap)
{
  ...
  // Ignore this directory if it contains a AMENT_IGNORE file.
  const path ament_ignore_path = _packagePath / "AMENT_IGNORE";
  if (exists(ament_ignore_path))
    return;
...
or_urdf::AmentFinder::AmentFinder():
  mWorkspaces(getWorkspaces())
{
}

std::vector<or_urdf::AmentFinder::Workspace> or_urdf::AmentFinder::getWorkspaces()
{
  std::vector<or_urdf::AmentFinder::Workspace> workspaces;

  // initialize workspaces
  // (from AmentResourceRetriever::getWorkspaces())
  using boost::filesystem::path;

  const char *ament_prefix_path = std::getenv("AMENT_PREFIX_PATH");
  if (!ament_prefix_path)
  {
    RAVELOG_WARN("The AMENT_PREFIX_PATH"
              " environmental variable is not defined. Did you source"
              " 'setup.bash' in this shell?\n");
    return workspaces;
  }

  // Split AMENT_PREFIX_PATH by the ':' path separator delimiter.
  std::vector<std::string> workspace_candidates;
  boost::split(workspace_candidates, ament_prefix_path, boost::is_any_of(":"));
  ...
  for (const std::string& workspace_path : workspace_candidates)
  {
    ...
    // Construct workspace
    or_urdf::AmentFinder::Workspace workspace;
    workspace.mPath = workspace_path;

    // Filter out directories that are missing the Ament marker file. If the
    // marker file exists, then we also read its contents to build a list of all
    // source directories.
    boost::filesystem::path marker_bpath = workspace_bpath / AMENT_MARKER;
...
std::string or_urdf::AmentFinder::find(const std::string & uri) const
{
  using boost::filesystem::path;
  ...
  {
    RAVELOG_ERROR("AmentFinder passed a non package:// path!\n");
    return "";
  }
  ...
  {
    // First check the 'build' or 'install' space.
    const path buildPath = path(workspace.mPath) / "share" / packageName / relativePath;
    if (boost::filesystem::exists(buildPath))
      return buildPath.native();
  ...
  RAVELOG_WARN("Could not find file '%s' under ament package '%s'!\n",
